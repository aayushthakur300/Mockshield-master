  ...
  return (
    <div className="landing-wrapper font-sans min-h-screen relative overflow-x-hidden">
      ...
      <style>{`
        ...
        /* BLACKOUT GRID: Smooth top-to-bottom fade using strict RGBA blacks */
        .danger-grid {
            position: fixed;
            top: 0;
            left: 0;
        ...
        .danger-text {
            color: var(--accent);
            text-shadow: 0 0 15px rgba(239, 68, 68, 0.6), 0 0 30px rgba(239, 68, 68, 0.4), 0 0 45px rgba(239, 68, 68, 0.2);
            animation: dangerPulse 3s infinite alternate;
        }

        @keyframes dangerPulse {
            0% { text-shadow: 0 0 15px rgba(239, 68, 68, 0.6), 0 0 30px rgba(239, 68, 68, 0.4); }
            50% { text-shadow: 0 0 20px rgba(239, 68, 68, 0.8), 0 0 40px rgba(239, 68, 68, 0.6), 0 0 60px rgba(239, 68, 68, 0.3); }
            100% { text-shadow: 0 0 15px rgba(239, 68, 68, 0.6), 0 0 30px rgba(239, 68, 68, 0.4); }
        }
        ...
        /* Essential utility to keep content strictly above the background grid */
        .content-layer {
            position: relative;
            z-index: 10;
        }
      ...
      <nav className="navbar-bg fixed top-0 w-full z-50 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            {/* MOBILE MENU TOGGLE BUTTON */}
            <button 
                className="md:hidden text-2xl text-[var(--text-main)] hover:text-red-500 transition-colors"
                onClick={toggleMenu}
            >
              <i className={`fa-solid ${isMenuOpen ? 'fa-xmark' : 'fa-bars'}`}></i>
            </button>
      ...
      <div className={`mobile-menu-overlay md:hidden ${isMenuOpen ? 'active' : ''}`}>
        <a href="#features" className="text-[var(--text-main)] text-xl font-bold uppercase tracking-widest hover:text-red-500 transition border-b border-[var(--border-color)] pb-2" onClick={() => setIsMenuOpen(false)}>Features</a>
        <a href="#demo" className="text-[var(--text-main)] text-xl font-bold uppercase tracking-widest hover:text-red-500 transition border-b border-[var(--border-color)] pb-2" onClick={() => setIsMenuOpen(false)}>Analysis</a>
        <a href="#contact" className="text-[var(--text-main)] text-xl font-bold uppercase tracking-widest hover:text-red-500 transition border-b border-[var(--border-color)] pb-2" onClick={() => setIsMenuOpen(false)}>Contact</a>
        <Link to="/dashboard" className="text-red-500 text-xl font-bold uppercase tracking-widest hover:text-[var(--text-main)] transition flex items-center gap-2" onClick={() => setIsMenuOpen(false)}>
            <i className="fa-solid fa-bolt"></i> Dashboard
        </Link>
      </div>
      ...
      {/* --- HERO SECTION --- */}
      <section className="relative pt-40 pb-20 min-h-screen flex items-center justify-center text-center px-6 content-layer">
        <div className="max-w-5xl mx-auto z-10">
          <div className="inline-block px-6 py-2 mb-8 border border-red-500/40 rounded-full bg-red-900/20 text-red-400 text-xs font-bold tracking-[0.2em] uppercase">
            V2.0 System Online
          </div>

          <h1 className="text-6xl md:text-[5.5rem] font-black mb-6 leading-[1.1] text-[var(--text-main)]">
            SUPREME AI <br />
            <span className="danger-text">INTERVIEW ARCHITECT</span>
          </h1>

          <p className="text-[var(--text-muted)] text-lg md:text-xl max-w-3xl mx-auto mb-12 leading-relaxed font-medium">
            Dual-engine mock interview simulation, resume gap analysis, and context-aware automated coaching. 
            Forensic transcript evaluations, and professional PDF dossiers.
          </p>
          ...
          <div className="flex flex-col md:flex-row gap-6 justify-center">
            {/* CTA: Launch Dashboard */}
            <Link to="/dashboard" className="btn-danger-glow px-12 py-5 rounded-lg text-lg flex items-center justify-center gap-3 w-full md:w-auto">
              <i className="fa-solid fa-microchip"></i> Launch MockShield
            </Link>
            
            {/* CTA: Explore Architecture RESTORED */}
            <a href="#features" className="px-12 py-5 rounded-lg text-lg font-bold border border-[var(--text-muted)] text-[var(--text-main)] hover:bg-[var(--accent)] hover:border-[var(--accent)] hover:text-white transition flex items-center justify-center gap-2 w-full md:w-auto">
              Explore Architecture <i className="fa-solid fa-arrow-down"></i>
            </a>
          </div>
        </div>
      ...
      {/* --- FEATURES GRID (20 Cards) --- */}
      <section id="features" className="py-24 bg-[var(--bg-section)] transition-colors duration-400 content-layer">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-black uppercase tracking-wide text-[var(--text-main)]">
              System <span className="text-red-500">Capabilities</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {features.map((feature, index) => (
              <div key={index} className="feature-card flex flex-col items-start">
                <i className={`fa-solid ${feature.icon} feature-icon-lg`}></i>
                <h3 className="text-xl font-bold text-[var(--text-main)] mb-3 feature-title">{feature.title}</h3>
                <p className="text-[var(--text-muted)] text-sm leading-relaxed font-medium feature-description">
                  {feature.desc}
                </p>
              </div>
            ))}
      ...
      {/* --- DEMO / ANALYSIS SECTION --- */}
      <section id="demo" className="py-24 relative overflow-hidden content-layer">
        <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-12 items-center">
          <div>
            <h2 className="text-3xl font-bold mb-6 text-[var(--text-main)]">
              Supreme AI <span className="text-red-500">LogicProbe</span>
            ...
            <Link to="/dashboard" className="mt-8 inline-block text-red-400 font-bold hover:text-red-500 transition tracking-wide">
              Initialize Forensic Simulation &rarr;
            </Link>
          </div>

          {/* --- CODE PREVIEW --- */}
          <div className="code-preview shadow-2xl shadow-red-900/10">
            <div className="flex items-center gap-2 mb-4 border-b border-[var(--border-color)] pb-2">
              <div className="w-3 h-3 rounded-full bg-red-500"></div>
              <div className="w-3 h-3 rounded-full bg-yellow-500"></div>
              <div className="w-3 h-3 rounded-full bg-green-500"></div>
      ...
      {/* --- CONTACT SECTION --- */}
      <section id="contact" className="py-16 border-t border-[var(--border-color)] bg-[var(--footer-bg)] transition-colors duration-400 content-layer">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <h3 className="text-3xl font-black mb-8 text-[var(--text-main)]">
            Connect with <span className="text-red-500">Us</span>
          </h3>
      ...
      {/* --- FIXED FOOTER --- */}
      <footer className="fixed bottom-0 w-full z-50 bg-[var(--footer-bg)] border-t border-[var(--border-color)] py-3 text-center transition-colors duration-400">
        <div className="max-w-7xl mx-auto px-6">
          <p className="text-[var(--text-muted)] text-sm font-bold tracking-wide">
            © 2026 MockShield • All Rights Reserved • Developed by Aayush Thakur
          </p>
        </div>
      </footer>
