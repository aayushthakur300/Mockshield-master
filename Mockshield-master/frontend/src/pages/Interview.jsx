...
const Interview = () => {
  ...
  useEffect(() => {
    const stateToSave = {
      ...
      hasStarted,
      isSubmitted,
      startTime // PERSIST: Save the timer start time
    };
  ...
  const terminateSession = useCallback(async (reason) => {
    ...
    try {
        const failurePayload = {
            questions: answers,
            topic: config.topic || "Mock Interview (Terminated)", 
            totalScore: 0, 
    ...
    navigate('/'); 
  }, [navigate, SESSION_KEY, answers, config.topic, violations, isSubmitted, getFormattedTimeTaken]);
  ...
  const finishInterview = async (finalAnswers) => {
    ...
    try {
      ...
      // Navigate and pass exact time and flags to the immediate live report
        ...
        <div className="space-y-3 sm:space-y-4 mb-4 sm:mb-8 text-slate-700 text-sm sm:text-lg leading-snug sm:leading-relaxed overflow-y-auto flex-1 pr-1 sm:pr-2">
           <ul className="list-none space-y-2 sm:space-y-3 bg-slate-50 p-3 sm:p-6 rounded-lg border border-slate-200">
             ...
             <li className="flex items-start gap-2 sm:gap-3 border-l-4 border-yellow-500 pl-2 sm:pl-3 bg-yellow-50 py-1 sm:py-2">
               ...
               <span><strong>Strict 30-Minute Limit</strong>: The interview duration is exactly 30 minutes. If time elapses, the session will immediately terminate without extra chances.</span>
             </li>
